...
The `/roles` controller is a deprecated wrapper over `AccessRole` that maps legacy role shapes to the unified store. `POST /roles` requires a client and creates the Access Role on that client's linked or auto-generated resource server. The wrapper can return more than application-scoped roles:
