...
6. Wait for the workflow to finish all three runtime checks: SSM command success, ALB target health, and the route probes — a public page, a protected page, and the OIDC authorization redirect `/_auth/login` answers with. Those probes connect directly to the ALB while preserving the canonical `DOCS_PUBLIC_BASE_URL` hostname for TLS, redirects, and `Origin` semantics, and they are the authoritative check of the three. The login probe is the only one that exercises the issuer: it is credential-free, does not follow the redirect, and requires an absolute HTTPS authorization endpoint on a host other than the docs host carrying `client_id`, this deployment's `redirect_uri`, `response_type=code`, `state`, `nonce`, and an S256 PKCE challenge. Without it, an unreachable issuer or a wrong discovery URL left both other probes green — the protected page is served entirely locally — and every reader failed at the next sign-in. Its diagnostics name the missing parameters rather than echoing the redirect, because that URL carries a live `state` and `nonce`. The target-health wait returns as soon as the target group reports `healthy`, and because the health check interval is longer than the symlink swap takes, a target healthy before the swap can still be reporting healthy just after it — so the wait does not prove which release answered. Read the probe output, not the wait.
...
- **Installer rollback on-host**: the installer stages the candidate release, health-checks it, atomically swaps `/opt/authifi-docs/current`, and restores the previous symlink automatically if the restart fails or if the active health check fails after it. A failed rollout therefore returns to the previous release unless there was no prior release yet. The one exception is release pruning, which runs last and is best effort: `release pruning failed; deployment is active` on stderr means the release is live and healthy and a stale directory could not be deleted, so investigate the host's disk rather than rolling back.
- **Instance replacement follow-up**: if Terraform replaces the instance, refresh `DOCS_INSTANCE_ID`, expect all sessions to be signed out because the host generated a new session key, and rerun the deploy workflow to restage a release on the new host. Do not start a deploy while the replacement is still applying: the workflow resolves `DOCS_INSTANCE_ID` as it runs, so it will either install onto the instance Terraform is about to terminate and report success while the site stays down, or fail against an ID that no longer exists. `infra/README.md` has the ordered runbook, including the `config.json` migration that causes a replacement.
